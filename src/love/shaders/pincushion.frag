...
vec4 effect(vec4 color_, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    vec2 p = pincushionDistortion(texture_coords, distort).xy;

    glitchSwap(p);
    glitchStatic(p);

    vec3 color = Texel(texture, p).rgb;
    return vec4(color.r * alpha, color.g * alpha, color.b * alpha, 1.0);
}
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    float alpha = pixel.a;
    vec2 p = texture_coords.xy;
    vec3 baseColor = Texel(texture, texture_coords).rgb;

    glitchSwap(p);
    glitchStatic(p);

    float amount = (0.5 * sin(time * PI) + vignetteIntensity);
    float vignette = distance(texture_coords, vec2(0.5));

    vec3 newColor = Texel(texture, p).rgb;

    vignette = mix(1.0, 1.0-amount, vignette);

    return vec4(mix(newColor.rgb, baseColor.rgb, vignette), 1.0);
}*/