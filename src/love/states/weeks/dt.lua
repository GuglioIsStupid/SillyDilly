...
return {
	enter = function(self, option)
		...
		if option ~= "pixel" then
			...
			images = {
				...
				notesMiddle = love.graphics.newImage(graphics.imagePath("dt/Bronzong_Gong_mechanic")),
	...
	initUI = function(self, option)
		...
		if not pixel then
			...
			sprites.middleReceptor = love.filesystem.load("sprites/dt/middleReceptor.lua")
			sprites.middleNote = love.filesystem.load("sprites/dt/middleNote.lua")
		else
		...
		for i = 1, 5 do
			if settings.middleScroll then 
				...
				if enemyArrows[i] then enemyArrows[i].x = 2929 end-- lol
			else
				boyfriendArrows[1].x = -925 + 165 * 1
				boyfriendArrows[2].x = -925 + 165 * 2
				boyfriendArrows[3].x = -895 + 165 * 3
			...
			if enemyArrows[i] then enemyArrows[i]:animate(noteList[i]) end

			if settings.downscroll then 
				if enemyArrows[i] then enemyArrows[i].sizeY = -1 end
				boyfriendArrows[i].sizeY = -1
			end

			enemyNotes[i] = {}
			boyfriendNotes[i] = {}
			if gfNotes[i] then gfNotes[i] = {} end
		end
	end,
		...
		for i = 1, #chart["notes"] do
			for j = 1, #chart["notes"][i]["sectionNotes"] do
				...
				if mustHitSection and noteVer ~= "Hurt Note" then
					...
					if noteType >= 4 and noteType < 8 then
					   	local id = noteType - 3
	...
	updateUI = function(self, dt)
		...
		for i = 1, 5 do
			...
			if enemyArrow then enemyArrow:update(dt) end
			boyfriendArrow:update(dt)

			if enemyArrow and not enemyArrow:isAnimated() then
			...
			if enemyNote and #enemyNote > 0 then
