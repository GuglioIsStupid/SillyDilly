...
return {
	...
	load = function(self)
		...
		for i = 1,3 do
			if _G["enemy" .. i] then _G["enemy" .. i]:animate("idle") end
			_G["boyfriend" .. i]:animate("idle")
		end
		...
		if not camera.points["enemy"] then 
			if enemy then
				camera:addPoint("enemy", -enemy1.x - 100, -enemy1.y + 75) 
			end
		end
	...
	update = function(self, dt)
		...
		_G["enemy" .. i]:update(dt)
		_G["boyfriend" .. i]:update(dt)
		_G["boyfriend" .. i]:beat(beatHandler.getBeat())
		_G["enemy" .. i]:beat(beatHandler.getBeat())
		end
		...
		for i = 1, 4 do
			...
			if #enemyNote > 0 then
				if (enemyNote[1].time - musicTime <= 0) then
					...
					if enemyNote[1].ver ~= "GF Sing" then
						...
						_G["enemy" .. i].lastHit = musicTime
						end
					else
						if enemyNote[1]:getAnimName() == "hold" or enemyNote[1]:getAnimName() == "end" then
							if useAltAnims then
								if girlfriend.holdTimer > enemy1.maxHoldTimer then girlfriend:animate(curAnim .. " alt", _psychmod and true or false) end
							else
								if girlfriend.holdTimer > enemy1.maxHoldTimer then girlfriend:animate(curAnim, (_psychmod and true or false)) end
							end
						else
							if useAltAnims then
				...
				if #boyfriendNote > 0 then
					if (boyfriendNote[1].time - musicTime <= 0) then
						...
						if boyfriendNote[1]:getAnimName() == "hold" or boyfriendNote[1]:getAnimName() == "end" then
							for i = 1,3 do
							if _G["boyfriend" .. i].holdTimer >= _G["boyfriend" .. i].maxHoldTimer then _G["boyfriend" .. i]:animate(curAnim, false) end
							end
						else
							for i = 1,3 do
								_G["boyfriend" .. i]:animate(curAnim, false)
							end
						end

						for i = 1,3 do 

							_G["boyfriend" .. i].lastHit = musicTime
						end
						if boyfriendNote[1]:getAnimName() ~= "hold" and boyfriendNote[1]:getAnimName() ~= "end" then 
			...
			if input:pressed(curInput) then
				...
				if not success then
					...
					_G["boyfriend" .. i]:animate(curAnim .. " miss", false)
					end
					score = score - 10
					combo = 0
					health = health - 0.135
